...
execute as @a[tag=techsword] at @s unless entity @e[type=interaction,tag=techsword.input.6,distance=..3] anchored eyes positioned ^ ^ ^ run summon interaction ~ ~-1.5 ~ {Tags:["techsword.input.6"],width:1,height:1}
