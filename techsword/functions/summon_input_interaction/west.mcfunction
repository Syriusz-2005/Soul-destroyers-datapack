...
execute as @a[tag=techsword] at @s unless entity @e[type=interaction,tag=techsword.input.4,distance=..3] anchored eyes positioned ^ ^ ^ run summon interaction ~-1 ~-0.5 ~ {Tags:["techsword.input.4"],width:1,height:1,Invulnerable:true}
